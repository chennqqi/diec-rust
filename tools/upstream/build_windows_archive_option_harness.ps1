...
$HarnessSource = Resolve-ExistingPath (
    Join-Path $ScriptRoot "archive_harness_main.cpp"
) "archive-option harness source"
...
$LocalHarness = Join-Path $ConsoleBuild "archive_harness_main.cpp"
$HarnessMakefile = Join-Path $ConsoleBuild "Makefile.ArchiveOption.Release"
...
$BuildCommand = @(
    ...
    "cd /d `"$ConsoleBuild`"",
    "nmake /NOLOGO /F Makefile.ArchiveOption.Release $HarnessTarget"
) -join " && "
& $env:ComSpec /d /c $BuildCommand
if ($LASTEXITCODE -ne 0) {
    throw "archive-option harness build failed: $LASTEXITCODE"
}
$Timer.Stop()
...
$Manifest = [ordered]@{
    ...
    source_hashes = [ordered]@{
        ...
        harness = [ordered]@{
            path = "tools/upstream/archive_harness_main.cpp"
