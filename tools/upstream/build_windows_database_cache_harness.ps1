...
$HarnessSource = Resolve-ExistingPath(
    (Join-Path $ScriptRoot "database_cache_harness_main.cpp")
) "shared harness source"
$AdapterSource = Resolve-ExistingPath(
    (Join-Path $ScriptRoot "database_cache_harness_windows_adapter.cpp")
) "Windows harness adapter"
$CompatHeader = Resolve-ExistingPath(
    (Join-Path $ScriptRoot "windows_database_cache_compat\unistd.h")
...
$PatchedMakefile = $OriginalMakefile.Replace(
    ...
    $SourceMatch.Value,
    "database_cache_harness_windows_adapter.cpp"
).Replace(
    $OriginalTarget,
    $HarnessTarget
)
...
$LocalHarness = Join-Path $ConsoleBuild "database_cache_harness_main.cpp"
$LocalAdapter = Join-Path $ConsoleBuild `
    "database_cache_harness_windows_adapter.cpp"
$LocalCompat = Join-Path $ConsoleBuild "unistd.h"
$HarnessMakefile = Join-Path $ConsoleBuild "Makefile.Harness.Release"
[System.IO.File]::Copy($HarnessSource, $LocalHarness, $true)
[System.IO.File]::Copy($AdapterSource, $LocalAdapter, $true)
[System.IO.File]::Copy($CompatHeader, $LocalCompat, $true)
[System.IO.File]::WriteAllText(
    $HarnessMakefile,
    $PatchedMakefile,
    [System.Text.UTF8Encoding]::new($false)
)
$HarnessObject = Join-Path $ConsoleBuild `
    "release\database_cache_harness_windows_adapter.obj"
...
$BuildCommand = @(
    "call `"$VsDevCmd`"",
    "cd /d `"$ConsoleBuild`"",
    "nmake /NOLOGO /F Makefile.Harness.Release $HarnessTarget"
) -join " && "
...
if ($BuildExitCode -ne 0) {
    throw "Windows cache harness build failed: $BuildExitCode"
}
...
$OutputBinary = Resolve-ExistingPath $OutputBinary "output cache harness"
...
$Result = [ordered]@{
    ...
    source_hashes = [ordered]@{
        ...
        windows_adapter = Get-Sha256 $AdapterSource
        windows_compat_header = Get-Sha256 $CompatHeader
    }
    artifact = [ordered]@{
