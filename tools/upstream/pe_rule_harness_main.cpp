// Project-generated research harness for a fixed PE rule and real XPE context.
...
constexpr const char *RULE_PATH =
    "/opt/die-source/Detect-It-Easy/db/PE/compiler_Cygwin32.4.sg";
...
QJsonObject serializeMemoryMap(const XBinary::_MEMORY_MAP &memoryMap)
{
    ...
    QJsonObject output;
    output.insert("file_type", "pe");
    output.insert("endian", "little");
    output.insert(
        "module_address",
        QString::number(
...
)
{
    ...
    buffer.setProperty("FileName", id + ".exe");
    if (!buffer.open(QIODevice::ReadOnly)) {
        *error = QString("cannot open PE fixture: %1").arg(id);
        return {};
    }

    XPE pe(&buffer);
    const bool parserValid = pe.isValid();
    XBinary::_MEMORY_MAP memoryMap = pe.getMemoryMap();
    const qint64 entryPointOffset = pe.getEntryPointOffset(&memoryMap);
    XBinary::PDSTRUCT state = XBinary::createPdStruct();
    Binary_Script::OPTIONS options = {};
    PE_Script script(&pe, XBinary::FILEPART_HEADER, options, &state);

    QScriptEngine engine;
    QScriptValue object = engine.newQObject(&script);
    engine.globalObject().setProperty("PE", object);
    ...
    output.insert("pe_script_error", XBinary::getPdStructErrorString(&state));
    return output;
}
...
int main(int argc, char *argv[])
{
    ...
    if (argc != 2) {
        std::fprintf(stderr, "usage: diec-pe-rule-harness <fixture.json>\n");
        return 2;
    }
