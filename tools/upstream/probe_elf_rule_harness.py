#!/usr/bin/env python3
"""Run and verify the pinned Qt5 ELF fixed-rule oracle harness."""
...
RULE_PATH = "/opt/die-source/Detect-It-Easy/db/ELF/protector_Burneye.2.sg"
RULE_SHA256 = "35461b495f056d98de9af44eda91df3c6412d22555b182834af9b6a68842d44c"
EXPECTED_IDS = [
    "burneye_elf32_entry_point_match",
    "burneye_elf32_entry_point_mismatch",
    "burneye_elf32_entry_point_truncated",
    "burneye_elf64_entry_point_match",
    "burneye_elf64_entry_point_mismatch",
...
def validate(
    ...
    for case_id in EXPECTED_IDS:
        ...
        if not isinstance(source, dict) or not isinstance(case, dict):
            ...
            continue
        for field in ("elf_class", "data_hex", "data_sha256"):
        ...
        is_match = case_id.endswith("_match")
        expected_detections = (
            [["protector", "Burneye", "1.0", ""]] if is_match else []
        )
        if case.get("parser_valid") is not True:
            failures.append(f"{case_id}.parser_valid")
        if case.get("entry_point_offset") != 256:
            failures.append(f"{case_id}.entry_point_offset")
        ...
        memory_map = case.get("memory_map")
        if (
            not isinstance(memory_map, dict)
            or memory_map.get("file_type") != "elf"
            or memory_map.get("endian") != "little"
            or not isinstance(memory_map.get("records"), list)
        ):
            failures.append(f"{case_id}.memory_map")
        if case.get("elf_script_error") != "":
            failures.append(f"{case_id}.elf_script_error")
        if "error" in case:
