// Project-generated research harness for a fixed APK rule and real XAPK
...
constexpr const char *RULE_PATH =
    "/opt/die-source/Detect-It-Easy/db/APK/protector_QDBH.2.sg";
...
)
{
    ...
    if (id.isEmpty() || dataHex.size() != data.size() * 2) {
        *error = "invalid APK fixture case";
        return {};
    }
    ...
    if (!buffer.open(QIODevice::ReadOnly)) {
        *error = QString("cannot open APK fixture: %1").arg(id);
        return {};
    }

    XAPK apk(&buffer);
    const bool parserValid = apk.isValid();
    XBinary::PDSTRUCT state = XBinary::createPdStruct();
    const QList<XArchive::RECORD> records = apk.getRecords(20000, &state);
    Binary_Script::OPTIONS options = {};
    APK_Script script(&apk, XBinary::FILEPART_HEADER, options, &state);

    QScriptEngine engine;
    QScriptValue object = engine.newQObject(&script);
    engine.globalObject().setProperty("APK", object);

    QJsonArray recordNames;
    for (const XArchive::RECORD &record : records) {
        recordNames.append(record.spInfo.sRecordName);
    }
    QJsonObject output;
    output.insert("id", id);
    ...
    output.insert("archive_record_count", records.size());
    output.insert("archive_record_names", recordNames);
    output.insert(
        "native_qdbh_present",
        script.isArchiveRecordPresent(QStringLiteral("assets/qdbh"))
    );
...
int main(int argc, char *argv[])
{
    ...
    if (argc != 2) {
        std::fprintf(stderr, "usage: diec-apk-rule-harness <fixture.json>\n");
        return 2;
    }
