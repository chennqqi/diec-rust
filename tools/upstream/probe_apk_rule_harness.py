#!/usr/bin/env python3
"""Run and verify the pinned Qt5 APK fixed-rule oracle harness."""
...
RULE_PATH = "/opt/die-source/Detect-It-Easy/db/APK/protector_QDBH.2.sg"
RULE_SHA256 = "cc20faadf1aec677679151a1997ea95184b265db2dbb1d4fcf56f0b62cead752"
EXPECTED_IDS = [
    "qdbh_archive_record_match",
    "qdbh_archive_record_case_mismatch",
    "qdbh_local_records_truncated",
]
EXPECTED_NAMES = {
    "qdbh_archive_record_match": ["classes.dex", "assets/qdbh"],
    "qdbh_archive_record_case_mismatch": ["classes.dex", "assets/QDBH"],
    "qdbh_local_records_truncated": ["classes.dex", "assets/qdbh"],
}
...
def validate(
    ...
    for case_id in EXPECTED_IDS:
        ...
        is_match = case_id != "qdbh_archive_record_case_mismatch"
        expected_detections = (
            [["protector", "QDBH", "", ""]] if is_match else []
        )
        if case.get("parser_valid") is not True:
            failures.append(f"{case_id}.parser_valid")
        if case.get("archive_record_count") != 2:
            failures.append(f"{case_id}.archive_record_count")
        if case.get("archive_record_names") != EXPECTED_NAMES[case_id]:
            failures.append(f"{case_id}.archive_record_names")
        ...
        if case.get("apk_script_error") != "":
            failures.append(f"{case_id}.apk_script_error")
        if "error" in case:
