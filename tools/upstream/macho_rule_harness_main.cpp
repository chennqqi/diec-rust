// Project-generated research harness for a fixed Mach-O rule and real XMACH
...
constexpr const char *RULE_PATH =
    "/opt/die-source/Detect-It-Easy/db/MACH/compiler_Rust.4.sg";
...
)
{
    ...
    const QString architecture = input.value("architecture").toString();
    const qint64 cpuType =
        static_cast<qint64>(input.value("cpu_type").toDouble());
    ...
    if (
        ...
        *error = "invalid Mach-O fixture case";
        return {};
    }
    ...
    if (!buffer.open(QIODevice::ReadOnly)) {
        *error = QString("cannot open Mach-O fixture: %1").arg(id);
        return {};
    }

    XMACH mach(&buffer);
    const bool parserValid = mach.isValid();
    XBinary::_MEMORY_MAP memoryMap = mach.getMemoryMap();
    const qint64 entryPointOffset = mach.getEntryPointOffset(&memoryMap);
    XBinary::PDSTRUCT state = XBinary::createPdStruct();
    Binary_Script::OPTIONS options = {};
    MACH_Script script(&mach, XBinary::FILEPART_HEADER, options, &state);

    QScriptEngine engine;
    QScriptValue object = engine.newQObject(&script);
    engine.globalObject().setProperty("MACH", object);

    QJsonObject output;
    output.insert("id", id);
    output.insert("architecture", architecture);
    output.insert("cpu_type", cpuType);
...
int main(int argc, char *argv[])
{
    ...
    if (argc != 2) {
        std::fprintf(stderr, "usage: diec-macho-rule-harness <fixture.json>\n");
        return 2;
    }
