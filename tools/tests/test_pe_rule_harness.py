...
GENERATOR = load_module(
    "generate_pe_rule_fixture",
    ROOT / "tools" / "corpus" / "generate_pe_rule_fixture.py",
)
PROBE = load_module(
    "probe_pe_rule_harness",
    ROOT / "tools" / "upstream" / "probe_pe_rule_harness.py",
)
FIXTURE_PATH = ROOT / "docs" / "research" / "data" / "pe-rule-fixture.json"
BASELINE_PATH = ROOT / "docs" / "research" / "data" / "pe-rule-qt5.json"
RULE_PATH = (
    ROOT
    / "upstream"
    / "Detect-It-Easy"
...
class PeRuleHarnessTests(unittest.TestCase):
    ...
    def test_probe_rejects_detection_and_input_drift(self):
        fixture = json.loads(FIXTURE_PATH.read_text(encoding="utf-8"))
        baseline = json.loads(BASELINE_PATH.read_text(encoding="utf-8"))
        ...
        self.assertIn(
            "cygwin32_entry_point_match.detections",
            failures,
        )
        self.assertIn(
            "cygwin32_entry_point_mismatch.data_sha256",
            failures,
        )
