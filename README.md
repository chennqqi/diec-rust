...
Phase 0 设计门禁已通过：固定上游基线、能力/源码/规则/行为分析、Rust 架构/API/C
ABI/测试方案设计，以及规则运行时和静态链接的验证性原型均已完成。项目现处于
Phase 1，建立 Cargo workspace 工程骨架、跨平台 CI、上游规则同步、差分测试基础设施
并冻结首版内部结果模型；公共 C ABI 仍保持实验状态。macOS 运行时基线采集作为
Phase 0 deferred 项在 Phase 1 并行完成。
