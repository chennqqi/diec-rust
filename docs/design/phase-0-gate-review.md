...
| P0-BLOCK-004 | Open | 上游 C++ 审计已完成 14 份技术证据文档（58 组件根许可证、XUCL/RAR/Brotli/Zstandard 来源追溯、产品源码闭包、runtime 规则资产、YARA/PEiD/signatures 来源）。本项目用 Rust 从零重写，不复制/翻译/链接上游 C++ 源码，因此上游 GPL/UnRAR/嵌入压缩许可证不传染 Rust 二进制。剩余缺口收窄为：(1) db* 规则资产（2,268 文件、22 PNG）分发许可书面评审；(2) YARA/PEiD/signatures 资产是否随 Rust 发布包分发；(3) NOTICE/SBOM 最终内容；(4) Rust crate SBOM 方案确认（Phase 1 crate 选定后构建）。详见 phase-0-review-preparation.md。 |
