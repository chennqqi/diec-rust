...
| P0-BLOCK-004 | Open | 上游 C++ 审计已完成 14 份技术证据文档。本项目是引擎，不包含检测规则（db* 规则由用户从 Detect-It-Easy 仓库自行获取），不实现 YARA/PEiD/signatures（源码证据证明这些是 GUI 专属功能，不进入 diec CLI）。Rust 从零重写不复制上游 C++ 源码，上游 GPL/UnRAR/嵌入压缩许可证不传染 Rust 二进制。剩余项为 Phase 1 常规工作：Rust crate 许可证清单（cargo deny/about）和 NOTICE 文件。详见 phase-0-review-preparation.md。 |
