...
| `P0-EXIT-002` | 基线覆盖主要格式和代表规则语法 | Not ready | 基础安全格式样本、七类专用规则差分、七种非 JPEG/PNG Image 分派、规则 priority、取消及 device/subdevice 边界已存在；闭集报告仍有 26 个 corpus-gap 行 |
...
| `P0-BLOCK-005` | Open | 68 行 × 4 平台 coverage report 已建立且无未分类 cell；Linux source-only 已清零，CAP-GAP-009/010/011/012 已由 device、规则排序、取消和七类 Image oracle 闭合；仍有 26 个 corpus-gap 行及三个平台各 68 个 platform-missing |
...
2. Linux source-only closure manifest 已完成；继续逐项收敛 26 个 corpus-gap 行，
