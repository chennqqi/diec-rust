...
| `P0-BLOCK-005` | Open | 68 行 × 4 平台 coverage report 已建立且无未分类 cell；Linux Qt5/Qt6 与 Windows Qt5 的 68 项均为 runtime-observed，source-only 与 corpus-gap 均为 0；Windows closure 绑定 23 份报告、2,438 次执行并以 path closure 关闭最后一行；CAP-GAP-008 的 Windows 部分 closed；macOS x86_64/Qt5 CLI-only bootstrap、oracle/cache candidate validator、64-exec 通用 CLI/corpus bundle、676-exec primary option/output/special matrix、1,092-exec remaining output/special matrix、36-exec release CLI database/error matrix 和 92-exec path/nested matrix collector/validator 已由同一 hash-bound workflow 固定，共保留 1,960 次执行的 3,920 个 raw stream，普通 output/special 已覆盖全部 26 个生成样本，但尚未在 Darwin 主机采集；平台 path profile、database archive/cache/permission 和 engine-only 矩阵仍缺，因此仍有 68 个 platform-missing |
