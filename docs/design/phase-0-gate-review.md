...
| `P0-EXIT-002` | 基线覆盖主要格式和代表规则语法 | Not ready | 26 个安全格式样本、七类专用规则差分已存在；闭集报告仍有 25 个 corpus-gap 行 |
...
| `P0-BLOCK-005` | Open | 68 行 × 4 平台 coverage report 已建立且无未分类 cell；仍需关闭 17 个 Linux source-only、25 个 corpus-gap 行及三个平台各 68 个 platform-missing |
...
2. coverage report 已完成闭集分类；优先为 17 个 Linux source-only 能力建立
   runtime corpus，并逐项收敛 25 个 corpus-gap 行，再扩展平台 oracle。
