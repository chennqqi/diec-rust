...
| `P0-BLOCK-005` | Open | 68 行 × 4 平台 coverage report 已建立且无未分类 cell；Linux Qt5/Qt6 与 Windows Qt5 的 68 项均为 runtime-observed，source-only 与 corpus-gap 均为 0；Windows closure 绑定 23 份报告、2,438 次执行并以 path closure 关闭最后一行；CAP-GAP-008 的 Windows 部分 closed；macOS x86_64/Qt5 CLI-only bootstrap、oracle/cache candidate validator、64-exec 通用 CLI/corpus bundle、676-exec primary option/output/special matrix、1,092-exec remaining output/special matrix、36-exec release CLI database/error matrix、92-exec path/nested matrix、34-exec ZIP-database CLI matrix、46-exec special-path CLI matrix、16-exec filesystem-path CLI matrix 和 10-exec large-directory CLI matrix collector/validator 已由同一 hash-bound workflow 固定，共保留 2,066 次执行的 4,132 个 raw stream；special-path fixture 动态记录 case/NFC-NFD alias、目录项字节名和非法 UTF-8 errno，CLI 矩阵再覆盖全部逻辑 UTF-8 拼写、目录枚举、显式顺序和 leading-dash；filesystem fixture/CLI 又固定 file/directory symlink、dangling link、self-cycle、非 root mode-000 权限和 64 层目录，self-cycle timeout 也作为观察保留；large-directory fixture/CLI 再固定 0/1/256/4096 flat 与 16×256 nested 的全部 8,449 个空文件、完整 prefix 数和 name-order projection，并明确不替代 long-path；普通 output/special 已覆盖全部 26 个生成样本，但所有候选均尚未在 Darwin 主机采集；root/ACL/ownership、long-path、TOCTOU、database cache/permission 和 engine-only 矩阵仍缺，因此仍有 68 个 platform-missing |
