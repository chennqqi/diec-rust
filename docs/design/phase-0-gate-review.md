...
| `P0-BLOCK-005` | Open | 68 行 × 4 平台 coverage report 已建立且无未分类 cell；Linux Qt5/Qt6 与 Windows Qt5 的 68 项均为 runtime-observed，source-only 与 corpus-gap 均为 0；Windows closure 绑定 23 份报告、2,438 次执行并以 path closure 关闭最后一行；CAP-GAP-008 的 Windows 部分 closed；macOS x86_64/Qt5 CLI-only bootstrap、oracle/cache candidate validator、64-exec 通用 CLI/corpus bundle、676-exec primary option/output/special matrix、1,092-exec remaining output/special matrix、36-exec release CLI database/error matrix、92-exec path/nested matrix、34-exec ZIP-database CLI matrix 和 46-exec special-path CLI matrix collector/validator 已由同一 hash-bound workflow 固定，共保留 2,040 次执行的 4,080 个 raw stream；special-path fixture 动态记录 case/NFC-NFD alias、目录项字节名和非法 UTF-8 errno，CLI 矩阵再覆盖全部逻辑 UTF-8 拼写、目录枚举、显式顺序和 leading-dash；普通 output/special 已覆盖全部 26 个生成样本，但所有候选均尚未在 Darwin 主机采集；symlink/cycle、permission、long-path、large-directory、TOCTOU、database cache/permission 和 engine-only 矩阵仍缺，因此仍有 68 个 platform-missing |
