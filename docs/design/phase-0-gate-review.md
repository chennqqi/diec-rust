...
| `P0-EXIT-002` | 基线覆盖主要格式和代表规则语法 | Not ready | 基础安全格式样本、七类专用规则差分、CLI 专用模式临界值、七种非 JPEG/PNG Image 分派、规则 priority、取消及 device/subdevice 边界已存在；Linux Qt5 的 68 项均为 runtime-observed，source-only 与 corpus-gap 均为 0，但 Linux Qt6、Windows、macOS 完整基线仍缺 |
...
| `P0-BLOCK-005` | Open | 68 行 × 4 平台 coverage report 已建立且无未分类 cell；Linux Qt5 的 68 项均为 runtime-observed，source-only 与 corpus-gap 均为 0；CAP-GAP-001/002/003/004/005/006/009/010/011/012 已闭合，其中 CAP-GAP-006 由固定源码证明 engine 解包 family 只有 ZIP/7Z/RAR/CAB/ISO9660 五类、五类默认/显式 archive 成对 oracle、100000/100001 精确记录边界、depth-64 与 33,554,546 bytes 累计展开量共同关闭；仍缺 Linux Qt6、Windows 和 macOS 三个平台各 68 个 platform-missing |
...
2. Linux source-only 与 corpus-gap 已清零；下一步建立 Windows、macOS 和完整
   Linux Qt6 baseline。
