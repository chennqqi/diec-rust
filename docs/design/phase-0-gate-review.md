...
| `P0-BLOCK-005` | Open | 68 行 × 4 平台 coverage report 已建立且无未分类 cell；Linux Qt5/Qt6 与 Windows Qt5 的 68 项均为 runtime-observed，source-only 与 corpus-gap 均为 0；Windows closure 绑定 23 份报告、2,438 次执行并以 path closure 关闭最后一行；CAP-GAP-008 的 Windows 部分 closed；macOS x86_64/Qt5 CLI-only bootstrap、oracle/cache candidate validator、26 个生成样本 × 双轮加 6 个通用 CLI case × 双轮的 64-exec raw bundle collector/validator 与 hash-bound 预执行计划已就绪，但尚未在 Darwin 主机采集；该 bundle 也不含 option/output/special/nested/path/database-error 和 engine-only 矩阵，因此仍有 68 个 platform-missing |
