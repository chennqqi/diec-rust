...
| 0015 | warm、file-content-nonresident-metadata-warm、dedicated system-cold 三层，拒绝通用 cold | dedicated authority/isolation、Windows/macOS 策略 |
...
- ADR 0015 的 per-file `mincore=0` 不能外推为 system-cold；当前工具不得通过
  ...
  的 warm/file-content 配对，但不能外推到 dedicated system-cold。
