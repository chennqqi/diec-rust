...
- **当前证据**：固定 Linux Qt5 archive oracle 已在 256 MiB/1 CPU/30 s 外部边界
  ...
  Copy/LZMA/LZMA2/BZip2/Deflate 五种 coder、x86/ARM64-BL BCJ+LZMA2 filter
  链与 CAB Store/MSZIP 都能到达 PDF child；未覆盖的 PPMd7/Deflate64、
  ARM64-BCJ ADRP/BCJ2/AES、CAB LZX/Quantum 与其他 backend 仍需共享同一
  预算。详见
...
- **验证**：Rust 同 bytes/options 成对报告后冻结回归阈值，持续 trend；补
  cold/affinity、Rust 成对/发行包 size 和目标平台。
