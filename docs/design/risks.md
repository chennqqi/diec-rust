...
- **当前证据**：Boa 与原始 QuickJS 都拒绝 Nintendo 规则；QuickJS 的单点、等长、
  ...
  27 个向量已确认 compare/find 在 `%&`、DEL、leading `+` 和 invalid suffix
  上存在不同语义；Rust context-free compare 当前差分 16/16。全调用点 inventory、
  format-specific memory-map、完整 find、Qt 6 和 HostApi 仍未验证。
