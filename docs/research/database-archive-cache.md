...
本轮只证明 Linux Qt5 qmake/CMake 固定 oracle。engine API 的 cache hit、
stale-content、corrupt/truncated cache 和 cancellation 尚未由专用 harness
运行，不从 CLI 结果外推。
...
cache-disabled 行为。engine cache 的精确 stale/corrupt/cancel 行为仍需专用
Qt5 harness 后才能决定是否进入低层兼容 profile。
...
- engine `bUseCache=true` 的首次保存、cache hit、同 size/mtime 内容替换；
- bad magic/version/engine、截断 record、超大 record count 和 cancellation；
