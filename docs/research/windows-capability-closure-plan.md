...
- 19 份 Windows runtime 报告的完整 SHA-256；
- 每份报告的 source/platform 身份和命名 summary facts；
- 2,212 次 Windows 进程执行，其中 legacy/archive dispatch 86 次执行、
...
| 分类 | 行数 |
| --- | ---: |
| Evidence complete | 64 |
| Partial | 1 |
| Missing | 3 |
| Total | 68 |

所有行均恰好分类一次，但仍有 4 行需要 closure，因此
...
### 4.1 Archive nested engine boundaries：3 行
...
以及 depth/cumulative expansion limit。它们不能由 release CLI
...
1. Windows archive nested engine harnesses：处理 3 个 missing；
