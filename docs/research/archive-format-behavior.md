# 7Z coder/filter、RAR4、CAB Store/MSZIP 与 ISO9660 archive 解包行为
...
固定 Linux x86_64 Qt5 engine harness 对十一个项目生成样本给出一致的正向结果：

- 7Z Copy/LZMA/LZMA2/BZip2/Deflate、x86 BCJ+LZMA2、ARM64-BCJ+LZMA2
  BL 分支、RAR4 store、CAB
  Store/MSZIP、ISO9660 的默认 engine 模式都不展开成员；
- 显式设置 `bIsArchivesScan` 后，十一个容器都产生恰好一个 `PDF / Stream`
...
这组结果增加了 7Z 五种单 coder、x86 BCJ+LZMA2 filter 链与
ARM64-BCJ+LZMA2 BL 分支、RAR4 store、CAB Store/MSZIP 与 ISO9660 的正向
corpus 证据，但不关闭
...
7Z PPMd7/Deflate64、ARM64-BCJ ADRP/BCJ2/AES、RAR 的压缩算法、CAB LZX/Quantum、
系统化畸形矩阵及跨平台行为仍未验证。
...
- `sevenzip_arm64_bcj_lzma2_member_reaches_pdf_rules`
...
| Fixture manifest | `db257aa27e19d5dc74cded1875728200297579ea39905f1c9f1120998f0957d7` |
...
| `pdf-member-arm64-bcj-lzma2.7z` | 7Z LZMA2 → ARM64 BCJ BL → `payload.pdf` | 312 | `281e79f450fd44c1dfbfaeb2484a85ad46af603d3690fc10cc868374125c32d1` |
...
前十个成员使用 331-byte payload，SHA-256 都是
`47bd96bd99d3fd9d9edf09151f7c62999aaf71ed599bd975db9e46c4d6ef5d92`。
ARM64 case 在 PDF EOF 后追加对齐 BL 指令，payload 为 336 bytes，SHA-256 是
`09c1c7ab1053428cc0f013e1cd782b508935c996086d55640645949725e088b6`。
生成器测试逐字节复验 size/hash，使用 Python 标准库独立解压七个压缩 stream，
并检查格式头、7Z Start/Next Header CRC、RAR header CRC、CAB size/压缩类型；
BCJ+LZMA2 使用同一标准 filter 链独立还原，MSZIP 的 `CK` + raw deflate 数据
由 Python `zlib` 独立还原。ARM64 case 的 BL 正向/逆向向量分别固定为
`0x94000002 → 0x94000055 → 0x94000002`，并证明 LZMA2 解压后得到已转换字节；
...
| --- | --- | --- | --- |
| 7Z Copy/LZMA/LZMA2/BZip2/Deflate/x86 BCJ+LZMA2 | `Binary / 7-Zip` | 0 Stream | 1 × 331-byte `PDF / Stream` |
| 7Z ARM64-BCJ+LZMA2 BL | `Binary / 7-Zip` | 0 Stream | 1 × 336-byte `PDF / Stream` |
...
除 ARM64 case 为字符串 `"336"` 外，archive child 的 size 都是 `"331"`；
规则检测名严格为
`["PDF", "HeaderComment"]`。每个样本的 `default == release_default`，
`archive == archive_aggressive`，比较对象是未经规范化的 stdout/stderr
原始字节，不只是摘要。

完整 44 次执行的原始 stream 以 SHA-256 为键，经 `zlib+base64` 去重嵌入报告；
...
本实验只证明五种 7Z 单 coder、x86 BCJ+LZMA2 filter 链、ARM64-BCJ+LZMA2
的 BL 分支、RAR4 store、CAB Store/MSZIP 与 ISO9660 的合法单成员正例，不证明：

- 通用 Archive 的自动/强制分派见
  [`generic-archive-dispatch-reachability.md`](generic-archive-dispatch-reachability.md)；
  NPM 的直接检测、公共自动回退和强制分支见
  [`npm-dispatch-reachability.md`](npm-dispatch-reachability.md)；
- 7Z 的 PPMd7、Deflate64、ARM64-BCJ ADRP/BCJ2 filter 与 AES 组合；
