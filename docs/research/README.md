...
- [`archive-format-behavior.md`](archive-format-behavior.md)：
  ...
  Store/MSZIP、ISO9660 正向解包、7Z 七种基础 coder+AES 与 x86/ARM64
  filter+AES 成功密码契约、Copy/PPMd7 错误密码残留输出、
...
- [`data/archive-format-engine-qt5.json`](data/archive-format-engine-qt5.json)：
  二十九个 archive/coder 样本的 default/release/archive/aggressive 原始输出、
  三十个 7Z AES 直接密码 case、固定身份和结构化摘要。
