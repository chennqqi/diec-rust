...
和合法/非法 range 均有固定 33-case Qt5 engine 证据；不安全的 silent success
...
相等；全部平台 cell 有已知状态；Linux 四类计数保持 50/18/0/0；其他三个平台
各保持 68 个 `platform_missing`；九个开放 gap 均映射到已知能力；所有
...
2. 逐项收敛剩余九个 corpus/platform gap，而不是只增加 happy-path 样本；
