...
legacy/archive dispatch 的 86 次执行、debug-data paired harness 双轮及
archive-option matrix 128 次执行，二十批共 2,340 次
...
现已把这 68 行逐项审计为 65 `evidence_complete`、1 partial、2 missing，
机器报告 SHA-256 为
`0e2d7c25177281dbccecd15d8b52c39f84dd8c9a3f4440a69d0d6a10703c3451`。
该报告绑定上述 20 份 Windows runtime 证据并为 3 个开放行给出验收实验；
...
- Windows 的独立 closure 审计为 65 complete/1 partial/2 missing，但尚未
...
Windows 的 65/1/2 closure 由独立生成器验证；在其达到 68/0/0 并显式接入
