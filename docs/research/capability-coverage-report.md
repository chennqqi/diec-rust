...
相等；全部平台 cell 有已知状态；Linux 四类计数保持 38/19/10/1；其他三个平台
...
1. 为 11 个 Linux source-only 能力建立固定 runtime corpus；
