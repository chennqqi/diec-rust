...
5 个确定性样本、4 种模式在固定 qmake/CMake 两个 oracle 上共运行 40 次。
...
`i < 100000` 上限。默认分支在扫描后用 `nCurrentIndex > nLimit` 判断，因此
按源码可扫描到第 21 个符合条件的成员，而不是严格最多 20 个。aggressive
受循环硬上限限制，最多访问 100000 个 entry。
...
- resource 的 `nLimit` 默认 20、aggressive 为 2000。条件使用
  `nCurrentIndex <= nLimit`，且计数只在实际扫描后增加，因此同样存在
  0-based 的“最多 21 个”默认边界；overlay 不受该判断约束。
...
- 直接调用 engine 并启用 `bIsArchivesScan` 的运行基线；
- ZIP/7Z/RAR/CAB/ISO9660 各自的解包错误、encrypted entry、重复名称和 metadata；
- 第 20/21/22 个 resource/archive 成员的边界实验；
- 多层 resource/overlay 链及实际最大栈深、取消、超时和内存峰值；
- 非 PE 格式的 overlay；
