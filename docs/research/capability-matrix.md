...
7Z Copy/LZMA/LZMA2/BZip2/Deflate、BCJ+LZMA2、RAR4 store、CAB Store/MSZIP
与 ISO9660 的 engine-only
archive 正向解包见
...
- `CAP-NEST-003`：archive 解包由独立 `bIsArchivesScan` 控制，发布 CLI 不设置它；ZIP 和
  ...
  证明五种 7Z 单 coder、BCJ+LZMA2 filter 链、RAR4 store、CAB Store/MSZIP
  与 ISO9660 默认与发布 CLI 逐字节相同且均不展开，显式
...
- `CAP-NEST-009`：固定源码没有独立嵌套 depth 或全 scan 累计展开字节状态；
  ...
  状态为 Observed with gaps；7Z Copy/LZMA/LZMA2/BZip2/Deflate、
  BCJ+LZMA2、RAR4 store、CAB Store/MSZIP 与 ISO9660 正例已经固定，但
...
- archive aggressive 100000、ZIP 1 MiB/843.58:1 与首轮压缩/加密/畸形边界
  ...
  五种 7Z 单 coder、BCJ+LZMA2 filter 链、RAR4 store、CAB Store/MSZIP 与
  ISO9660 正例、NPM 自动/强制分派 quirk 以及 generic
