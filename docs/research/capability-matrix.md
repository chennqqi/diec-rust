...
| `CAP-CLI-OUT-001` | `-x` | `--xml` | XML | Observed；Linux Qt5/Qt6 与 Windows Qt5 全 26 样本 |
| `CAP-CLI-OUT-002` | `-j` | `--json` | JSON | Observed；Linux Qt5/Qt6 与 Windows Qt5 全 26 样本，JSON tree 三方一致 |
| `CAP-CLI-OUT-003` | `-c` | `--csv` | CSV | Observed；Linux Qt5/Qt6 与 Windows Qt5 全 26 样本；normal scan 多格式开关中优先级最高 |
| `CAP-CLI-OUT-004` | `-t` | `--tsv` | TSV | Observed；Linux Qt5/Qt6 与 Windows Qt5 全 26 样本 |
| `CAP-CLI-OUT-005` | `-p` | `--plaintext` | plain text | Observed；Linux Qt5/Qt6 与 Windows Qt5 全 26 样本 |
...
- archive aggressive 100000、ZIP 1 MiB/843.58:1 与首轮压缩/加密/畸形边界，
  ...
  顺序/重名/空成员过滤、ISO9660 17-field 单侧双端序冲突已固定；
  其他算法、剩余字段、ISO path-table location/多字段组合冲突与算术 wrap、
  更大或混合失败记录图、最大深度和总解压资源限制仍待扩展；
...
- Windows Qt5 已完成 26-sample scan、26-sample 普通 output 和 26-sample
  ...
  Linux Qt5/Qt6 的同 26-sample output 矩阵也已完成，除 7 个 PE64 Qt6
  stderr 外 stdout/JSON tree/有效性/priority 均满足精确差分契约；
  ...
  发现路径；5-case ADS 矩阵固定显式 named stream 与目录枚举边界；17-case
  ZIP database 矩阵固定完整/空/截断、重复/`..` entry 和根前缀；19-case
  engine cache/DACL harness 又固定 stale/corrupt/cancel/write/read denial；
  UNC、精确 namespace 上限、symlink/reparse cycle、domain/group DACL、
  network share/EFS/integrity level、其他 engine-only 及跨平台扩展仍缺失。
...
现有 16 份报告支持 53 complete、8 partial、7 missing。该 53 行只是 closure
内部结论；15 个开放行关闭并经 coverage builder 接纳前，Windows 的 68 个
platform-missing 不变。
...
再固定规则层、init/include、priority 与 mode/type gate 五行，十个 canonical
case 和 14 条关系均与 Linux Qt5 相同。
