...
- `CAP-NEST-003`：archive 解包由独立 `bIsArchivesScan` 控制，发布 CLI 不设置它；ZIP 和
  ...
  无密码不产生 child，直接 `XSevenZip` 正确密码可还原同一 PDF。
...
- `CAP-NEST-009`：固定源码没有独立嵌套 depth 或全 scan 累计展开字节状态；
  ...
  ISO9660 正例、7Z LZMA2+AES 密码边界及 CAB LZX/Quantum 普通/激进失败边界
  已经固定，但
...
- archive aggressive 100000、ZIP 1 MiB/843.58:1 与首轮压缩/加密/畸形边界
  ...
  BCJ2+LZMA2 no-branch/E8/E9/JCC filter 链、7Z LZMA2+AES 密码边界、RAR4
  store、CAB
