...
| `CAP-CLI-IN-003` | 目录枚举 | positional directory | Observed | 无条件 depth-first；Linux 跟随 file/dir symlink、alias 不去重、权限错误静默，self-cycle 依赖 OS link 上限；flat/nested 4096 完整；TOCTOU/跨平台仍缺 |
...
- Linux Qt5 UTF-8、首轮非 UTF-8/特殊 filename、symlink/权限/depth-64/
  self-cycle、flat/nested 4096 与 CLI cancellation 接线已固定；仍缺 TOCTOU、
  locale/filesystem，
